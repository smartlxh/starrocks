...
public class ReplicationAction extends RestBaseAction {
    ...
    public void executeWithoutPassword(BaseRequest request, BaseResponse response) throws AccessDeniedException {
        ...
        if (type.equals("cancel_all")) {
            ...
            List<ReplicationJob> running = replicationMgr.getRunningJobs();
            List<ReplicationJob> aborted = replicationMgr.getAbortedJobs();
            List<ReplicationJob> committed = replicationMgr.getCommittedJobs();

            List<String> abortedIds = aborted.stream().map(ReplicationJob::getJobId).collect(Collectors.toList());
            List<String> committedIds = committed.stream().map(ReplicationJob::getJobId).collect(Collectors.toList());

            JsonObject root = new JsonObject();
            addIdArray(root, "abortedIds", abortedIds);
            addIdArray(root, "committedIds", committedIds);
            addReplicationJobs(root, "running", running);
    ...
    private void addReplicationJobs(JsonObject root, String name, List<ReplicationJob> running) {
