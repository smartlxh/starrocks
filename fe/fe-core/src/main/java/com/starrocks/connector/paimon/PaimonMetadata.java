...
public class PaimonMetadata implements ConnectorMetadata {
    ...
    private void updatePartitionInfo(String databaseName, String tableName) {
        ...
        if (paimonNativeCatalog instanceof CachingCatalog) {
            try {
                List<PartitionEntry> partitions = ((CachingCatalog) paimonNativeCatalog).listPartitions(identifier);
                for (PartitionEntry entry : partitions) {
                    InternalRowPartitionComputer partitionComputer
                            = FileStorePathFactory.getPartitionComputer(
                            RowType.of(partitionColumnTypes.toArray(new DataType[0])), null, true);
                    // partitionPath can be like f0=a/f1=b, in which f0/f1 is fixed value rather than partition key
                    String partitionPath = PartitionPathUtils.generatePartitionPath(
                            partitionComputer.generatePartValues(entry.partition()));
                    String[] partitionValues = Arrays.stream(partitionPath.split("/"))
                            .map(part -> part.split("=")[1])
                            .toArray(String[]::new);
                    Partition partition = getPartition(entry.recordCount(),
                            entry.fileSizeInBytes(), entry.fileCount(),
                            partitionColumnNames, partitionColumnTypes, partitionValues,
                            Timestamp.fromEpochMillis(entry.lastFileCreationTime()));
    ...
    public Statistics getTableStatistics(OptimizerContext session,
        ...
        try (Timer ignored = Tracers.watchScope("GetPaimonTableStatistics")) {
            ...
            if (!statistics.isPresent() || statistics.get().colStats() == null
                    || statistics.get().mergedRecordCount().isEmpty()) {
                return defaultStatistics(columns, table, predicate, limit);
            }
    ...
    public List<PartitionInfo> getPartitions(Table table, List<String> partitionNames) {
        ...
        if (table.isUnPartitioned()) {
            result.add(new Partition(paimonTable.getTableName(),
                    this.getTableUpdateTime(paimonTable.getDbName(), paimonTable.getTableName()), null, null, null));
            return result;
        }
